...
def nouns(inputtext, language):
	...
	wordsearch_nouns = list(set([word.lemma_.upper() for word in some_nouns_token]))#replace with lemmata, delete duplicates, make uppercase
	wordsearch_nouns_all = list(set([word.lemma_.upper() for word in noun_list]))#replace with lemmata, delete duplicates, make uppercase
